...
public record SQLiteDataMart(String dbUrl) {
    ...
    public SQLiteDataMart(String dbUrl) {
        this.dbUrl = dbUrl.startsWith("jdbc:sqlite:")
                ? dbUrl
                : "jdbc:sqlite:" + dbUrl;
    ...
    private void enableWALMode() {
        try (Connection conn = DriverManager.getConnection(dbUrl);
            ...
            System.err.println("❌ Error activando modo WAL: " + e.getMessage());
        }
    }
    ...
    public void save(JsonObject event) {
        try {
            // 1. Verificación básica: ¿Tiene fuente y timestamp?
            if (event == null || !event.has("ss") || !event.has("ts")) {
                return;
            }
            ...
            if (ss.contains("weather")) {
                // 2. Verificación de campos de clima
                if (!event.has("temp") ||
                        !event.has("location") ||
                ...
                WeatherEvent w = new WeatherEvent(
                        ts,
                        ss,
                        0,
                        event.get("temp").getAsDouble(),
                        event.get("humidity").getAsInt(),
                        0.0,
                        event.get("description").getAsString(),
                        event.get("location").getAsString(),
                        0.0,
                        0.0
                );
                ...
                // 3. Verificación de campos de satélite
                if (!event.has("id")) {
                    return;
                }

                double lat = event.has("lat")
                        ? event.get("lat").getAsDouble()
                        : (event.has("latitude")
                        ? event.get("latitude").getAsDouble()
                        : 0.0);

                double lon = event.has("lon")
                        ? event.get("lon").getAsDouble()
                        : (event.has("longitude")
                        ? event.get("longitude").getAsDouble()
                        : 0.0);

                SatelliteEvent s = new SatelliteEvent(
                        ts,
                        ss,
            ...
            System.err.println("⚠️ Evento ignorado (formato incorrecto): " + e.getMessage());
        }
    }
    ...
    public void addWeather(WeatherEvent w) {
        String sqlCurrent = """
            ...
            (location, description, temperature, humidity, timestamp)
            VALUES (?, ?, ?, ?, ?)
        """;

        String sqlHistory = """
            INSERT INTO weather_history
            (location, temp, timestamp)
            VALUES (?, ?, ?)
        """;
    ...
    public void addSatellite(SatelliteEvent sat) {
        ...
        try (Connection conn = DriverManager.getConnection(dbUrl);
            ...
            System.err.println("Error insertando satélite: " + e.getMessage());
        }
    }
    ...
    public void addSatellites(List<SatelliteEvent> satellites) {
        ...
        try (Connection conn = DriverManager.getConnection(dbUrl)) {
            ...
            System.err.println("Error en lote de satélites: " + e.getMessage());
        }
    }
    ...
    public WeatherEvent getLatestWeather(String location) {
        ...
        try (Connection conn = DriverManager.getConnection(dbUrl);
            ...
            System.err.println("Error consultando último clima: " + e.getMessage());
        }
        return null;
    }
    ...
    public List<SatelliteEvent> getActiveSatellites(int limit) {
        ...
        try (Connection conn = DriverManager.getConnection(dbUrl);
            ...
            System.err.println("Error consultando satélites: " + e.getMessage());
        }
        return satellites;
    }
}